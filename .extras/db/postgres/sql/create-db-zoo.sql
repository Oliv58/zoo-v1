...
-- (3) In Windows das Verzeichnis "C:/Zimmermann/volumes/postgres/tablespace/zoo" anlegen
--     Siehe .extras\compose\postgres\compose.yml
-- (4) 2. PowerShell:
--     cd .extras\compose\backend\postgres
--     docker compose exec db bash
--        chown postgres:postgres /var/lib/postgresql/tablespace
--        chown postgres:postgres /var/lib/postgresql/tablespace/zoo
...
-- (7) 2. PowerShell:
--     docker compose exec db bash
--        psql --dbname=postgres --username=postgres --file=/sql/create-db-zoo.sql
--        psql --dbname=zoo --username=zoo --file=/sql/create-schema-zoo.sql
--        exit
...
CREATE ROLE zoo LOGIN PASSWORD 'p';

-- https://www.postgresql.org/docs/current/sql-createdatabase.html
CREATE DATABASE zoo;

GRANT ALL ON DATABASE zoo TO zoo;

-- https://www.postgresql.org/docs/10/sql-createtablespace.html
CREATE TABLESPACE zoospace OWNER zoo LOCATION '/var/lib/postgresql/tablespace/zoo';
